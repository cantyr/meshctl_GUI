...
static uint16_t lightness_app_idx = APP_IDX_INVALID;

static int client_bind(uint16_t app_idx, int action)
{
	if (action == ACTION_ADD) {
		if (lightness_app_idx != APP_IDX_INVALID) {
		...
		if (lightness_app_idx == app_idx)
			lightness_app_idx = APP_IDX_INVALID;
	}
...
static bool send_cmd(uint8_t *buf, uint16_t len)
{
	...
	return net_access_layer_send(ttl, node_get_primary(node),
					target, lightness_app_idx, buf, len);
}
...
static void cmd_set(int argc, char *argv[])
{
	...
	n = mesh_opcode_set(OP_LIGHTNESS_SET, msg);
	lightness = (uint8_t *)&parms[0];
	msg[n++] = *lightness++;
	msg[n++] = *lightness;
	msg[n++] = trans_id++;
	lightness = (uint8_t *)&parms[1];
	msg[n++] = *lightness;
	parms[2] /= 5;
	lightness = (uint8_t *)&parms[2];
	msg[n++] = *lightness;
...
static const struct bt_shell_menu lightness_menu = {
	.name = "lightness",
	.desc = "Lightness Model Submenu",
	.entries = {
	{"target",		"<unicast>",			cmd_set_node,
						"Set node to configure"},
	{"get",			NULL,				cmd_get_status,
						"Get Lightness status"},
	{"lightness",	"<0-100> TT Delay",			cmd_set,
						"Send \"Set Lightness\" command"},
	{} },
};
